...
import Entry from 'draft-js-mention-plugin/lib/MentionSuggestions/Entry';
...
import defaultEntryComponent from 'draft-js-mention-plugin/lib/MentionSuggestions/Entry/defaultEntryComponent';
//import Base from 'draft-js-mention-plugin/lib/MentionSuggestions';
let tagTypes = ['event', 'before', 'after'];

// This is a ported copy of the original code.
// The original needed porting because it uses some weird invalid ES6 syntax that I think has something to do with TypeScript or Browserify.
// I may be able to delete this and use the original, so I am keeping this
// as a base class unless it becomes better to just change it and not
// inherit from it.
export class Base extends Component {
    static suggestionsHoc(Comp) {
        return props => {
            if (List.isList(props.suggestions)) {
                console.warn(
                    'Immutable.List for the "suggestions" prop will be deprecated in the next major version, please use an array instead'
                ); // eslint-disable-line no-console
            }

            return <Comp {...props} suggestions={fromJS(props.suggestions)} />;
        };
    }
    ...
    constructor(props) {
        ...
        this.onMentionFocus = this.onMentionFocus.bind(this);
        ...
        const selectionIsInsideWord = leaves.filter(leave => leave !== undefined).map(
            ({ start, end }) =>
                (start === 0 &&
                    ...
                    new RegExp(String.raw({ raw: `${escapeRegExp(this.props.mentionTrigger)}` }), 'g').test(
                        plainText
                    ) &&
...
export class MentionSuggestions extends Base {
    constructor(props){
        super(props);
    }
    onMentionSelect(mention){
        ...
        const mentionName = mention.get('name');
        if (tagTypes.includes(mentionName)){
            let editorState = this.props.store.getEditorState();
            const currentContent = editorState.getCurrentContent();
            const currentSelectionState = editorState.getSelection();
            const { begin, end } = getSearchText(editorState, currentSelectionState, this.props.mentionTrigger);
            ...
            this.props.store.setEditorState(EditorState.forceSelection(newEditorState, newContentState.getSelectionAfter()));
            this.props.onSearchChange({ value: mentionName });
            return;
        }
        // makes the entity show up correctly in draft.js editor (with type).
        let newMention = mention.set('name', `${mention.get('type')}: ${mention.get('data').get('originalName')}`);
        return super.onMentionSelect(newMention);
    }
}

export default MentionSuggestions.suggestionsHoc(MentionSuggestions);
